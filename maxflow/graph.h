...
Energy Minimization in Vision." Yuri Boykov and Vladimir Kolmogorov. In IEEE
...
Cuts." Pushmeet Kohli and Philip H.S. Torr International Conference on Computer
Vision (ICCV), 2005
...
private:
  ...
  struct node {
    ...
    int is_sink : 1; // flag showing whether the node is in the source or in the
                     // sink tree (if parent!=NULL)
    ...
    tcaptype tr_cap; // if tr_cap > 0 then tr_cap is residual capacity of the
                     // arc SOURCE->node otherwise         -tr_cap is residual
...
Graph<captype, tcaptype, flowtype>::get_nodes() {
  return nodes;
}
